...
public class AddableRegistryEntry<M, T extends Keyed, B extends PaperRegistryBuilder<M, T>> extends CraftRegistryEntry<M, T> implements RegistryEntry.Addable<M, T, B> {
    ...
    private WritableCraftRegistry<M, T, B> createRegistry(final Registry<M> registry) {
        return new WritableCraftRegistry<>(this, this.classToPreload, (SimpleRegistry<M>) registry, this.updater, this.builderFiller.asFactory(), this.minecraftToBukkit);
    }
	...
	// 1.21.1 api
	public B fillBuilder(Conversions a, TypedKey<T> b, M c) {
		return this.builderFiller.fill(a, c);
	}
