...
public final class RegistryTypeMapper<M, A> {
    ...
    private static <M, A> Function<RegistryEntry<M>, ? extends A> wrap(final BiFunction<? super NamespacedKey, M, ? extends A> byValueCreator) {
        return holder -> {
            ...
            return byValueCreator.apply(MCUtil.fromResourceKey(reference.registryKey()), reference.value());
        };
    }

    final Either<Function<RegistryEntry<M>, ? extends A>, BiFunction<? super NamespacedKey, M, ? extends A>> minecraftToBukkit;
    ...
    public RegistryTypeMapper(final Function<RegistryEntry<M>, ? extends A> byHolderCreator, final boolean supportsDirectHolders) {
        this.minecraftToBukkit = Either.left(byHolderCreator);
        this.supportsDirectHolders = supportsDirectHolders;
    }

    public A createBukkit(final RegistryEntry<M> minecraft) {
        return this.minecraftToBukkit.<Function<RegistryEntry<M>, ? extends A>>map(
            Function.identity(),
