...
final class AttributeMappings {
    ...
    private static final Map<UUID, NamespacedKey> ATTRIBUTE_MODIFIER_IDS = new HashMap<UUID, NamespacedKey>();

    AttributeMappings() {
    }

    @NotNull
    public static NamespacedKey uuidToKey(UUID uuid) {
        NamespacedKey key = ATTRIBUTE_MODIFIER_IDS.get(uuid);
        if (key != null) {
            return key;
        }
        return NamespacedKey.minecraft((String)uuid.toString());
    }

    private static void add(long msb, long lsb, String id) {
        UUID uuid = new UUID(msb, lsb);
        ATTRIBUTE_MODIFIER_IDS.put(uuid, NamespacedKey.minecraft((String)id));
    }
